...
export default function WidgetCard({ widget, rows, segments, editMode, onUpdate, onRemove }: Props) {
  ...
  return (
    <div className={`bg-slate-900 border rounded-xl overflow-hidden flex flex-col ${editMode ? 'border-blue-500/50' : 'border-slate-800'}`}>
      {/* Card Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-800">
        <div className="flex items-center gap-2 min-w-0">
          ...
          {editMode ? (
            <input
              ...
              className="bg-transparent text-sm font-medium focus:outline-none border-b border-slate-700 w-full"
            />
          ) : (
            <span className="text-sm font-medium truncate">{widget.title}</span>
          )}
          {seg && <span className="text-xs text-slate-600 flex-shrink-0">{seg.name}</span>}
        </div>
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
            ...
            <div className="flex gap-0.5 mr-1">
              {(['daily', 'weekly', 'monthly'] as Period[]).map(p => (
                <button
                  ...
                  className={`text-xs px-2 py-0.5 rounded transition ${period === p ? 'bg-blue-600 text-white' : 'text-slate-500 hover:text-white'}`}
                >
                  {PERIOD_LABELS[p]}
                </button>
              ))}
          ...
          {editMode && (
            <>
              <button onClick={() => setShowConfig(v => !v)} className="text-slate-500 hover:text-white text-xs px-2 py-1 rounded hover:bg-slate-800 transition">⚙</button>
              <button onClick={onRemove} className="text-slate-500 hover:text-red-400 text-xs px-2 py-1 rounded hover:bg-slate-800 transition">✕</button>
            </>
          )}
        </div>
      </div>
      ...
      {/* Config panel */}
      {showConfig && editMode && (
        <div className="bg-slate-800 border-b border-slate-700 p-3 grid grid-cols-2 gap-2">
          <div>
            <label className="text-xs text-slate-500 block mb-1">차트 타입</label>
          ...
          <div>
            <label className="text-xs text-slate-500 block mb-1">그룹 컬럼</label>
          ...
          <div className="col-span-2">
            <label className="text-xs text-slate-500 block mb-1">세그먼트</label>
      ...
      <div className="flex-1 p-4">
        {widget.type === 'kpi' && (
          <div className="flex flex-col justify-center h-full">
            <p className="text-slate-500 text-xs mb-1">{widget.valueCol}</p>
            <p className="text-3xl font-bold">{fmt(totalValue)}</p>
            <p className="text-xs text-slate-500 mt-1">{filteredRows.length.toLocaleString()}건</p>
          </div>
        )}
              ...
              <thead>
                <tr className="border-b border-slate-800">
                  {chartData[0] && Object.keys(chartData[0]).map(k => (
                    <th key={k} className="text-left py-1.5 px-2 text-slate-500 font-medium">{k}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {chartData.slice(0, 10).map((row, i) => (
                  <tr key={i} className="border-b border-slate-800/50">
                    {Object.values(row).map((v, j) => (
                      <td key={j} className="py-1.5 px-2 text-slate-300">{typeof v === 'number' ? v.toLocaleString() : String(v)}</td>
                    ))}
                  </tr>
                ))}
              </tbody>
        ...
        {chartData.length === 0 && widget.type !== 'kpi' && (
          <div className="flex items-center justify-center h-32 text-slate-700 text-xs">데이터 없음</div>
        )}
      </div>
    </div>
  )
